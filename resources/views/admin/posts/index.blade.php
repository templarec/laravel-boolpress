            ...
            <div class="col-md-8">
                ...
                <div class="card">
                    <div class="card-header"><a href="{{route('posts.show', ['slug' => $post->slug])}}"> {{$post->title}} </a> <span class="float-right p-1"><a href="{{route('admin.posts.edit', ['post' => $post->id])}}"><i class="far fa-edit"></i></a></span> <a class="p-1 float-right" onclick="event.preventDefault();
                                  document.getElementById('form-delete-post').submit();"><i class="far fa-trash-alt"></i></a> </div>
                    ...
                    <div class="card-body">
                        ...
                        <form  id="form-delete-post" action="{{route('admin.posts.destroy', ['post' => $post->id])}}" method="POST" style="display: none;">
                        @csrf
                        @method('DELETE')
                    </div>
                </div>
